...
On Grok: ignore the `model:` field. Spawn at most two extra seats. Require
Autowork / JustAMCP / CLI `--json` / `INCONCLUSIVE` — not screenshots.
Grok `code_execution` is not Blazium evidence.
